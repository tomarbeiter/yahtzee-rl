...
class YahtzeeSinglePlayerV8:
    ...
    def _make_reward(self, action, state):
        ...
        if type(action) is int and 5 in state[0]:
            return 100 if action == 11 else 1

        # Give bad rewards for scoring something other than straight or full house if it could score those:
        if type(action) is int:
            if action not in [8, 9, 10] and \
                (self.ytz.c_player.t_scorecard[8][0] != 0 or \
                self.ytz.c_player.t_scorecard[9][0] != 0 or \
                self.ytz.c_player.t_scorecard[10][0] != 0):
                return 1
        ...
        # Reward for scoring 3/4 of a kind:
        if type(action) is int and (action == 6 or action == 7):
            if self.ytz.c_player.t_scorecard[6][0] > 0 and action == 6:
                return 50
            elif self.ytz.c_player.t_scorecard[7][0] > 0 and action == 7:
                return 50
            else:
                return 1
        ...
        if type(action) is int and action == 12:
            if self.ytz.c_player.t_scorecard[12][0] >= 20:
                return 50
            else:
                return 1

        # Rolling rewards
        # Give bad reward for rolling when it could've scored a straight or full house
        if type(action) is list:
            if self.ytz.c_player.t_scorecard[8][0] != 0 or \
                self.ytz.c_player.t_scorecard[9][0] != 0 or \
                self.ytz.c_player.t_scorecard[10][0] != 0:
                return 1
